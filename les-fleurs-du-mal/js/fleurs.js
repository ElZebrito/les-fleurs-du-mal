  (function(){
    ...
    /* Nav + Hero entrée (remplace les CSS @keyframes heroIn) */
    ...
    function sr(sel, fromVars, staggerEach=0){
      gsap.utils.toArray(sel).forEach((el,i) => {
        gsap.from(el,{
          ...fromVars, duration:1.05, ease:'power3.out',
          delay:staggerEach*i,
    ...
    // Hero — watermark (grand chiffre romain) dérive vers le bas
    ...
    // Présentation — quote et texte à des vitesses différentes
    gsap.to('.pres-quote', {
      y:-40, ease:'none',
      scrollTrigger:{ trigger:'#presentation', start:'top bottom', end:'bottom top', scrub:1.5 }
    });
    gsap.to('.pres-text', {
      y:-20, ease:'none',
      scrollTrigger:{ trigger:'#presentation', start:'top bottom', end:'bottom top', scrub:2.5 }
    });

    // Poèmes — cartes avec légère profondeur alternée
    gsap.utils.toArray('.poem-card').forEach((card, i) => {
      gsap.to(card, {
        y: i % 2 === 0 ? -20 : -35, ease:'none',
        scrollTrigger:{ trigger:'#poemes', start:'top bottom', end:'bottom top', scrub:2 }
      });
    });

    // Navigation section — liens arrivent depuis le bas avec scrub
    gsap.utils.toArray('.section-link').forEach((link, i) => {
      gsap.from(link, {
        y:30, ease:'none',
        scrollTrigger:{ trigger:link, start:'top bottom', end:'center 70%', scrub:1.5 }
      });
    });

    // Nav — devient transparente en scrollant
    ScrollTrigger.create({
      start:'top -80',
      onUpdate(self){
